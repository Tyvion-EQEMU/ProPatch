...
async def _self_update_profetch(client, comp, installed_version: str | None) -> dict:
    ...
    ps_lines += [
        ...
        "",
        "# 4. Launch via Shell.Application.ShellExecute — identical to a user",
        "#    double-clicking the exe in Explorer. The new process is created by",
        "#    Windows Shell with a clean user-profile environment; it does NOT",
        "#    inherit the current process chain's env vars, so PyInstaller's",
        "#    internal coordination vars (_PYIBoot_HOME etc.) are absent and",
        ...
        "        Where-Object { $_ -match '^_PYI|^_MEI' }",
        "    foreach ($k in $keys) {",
        "        [System.Environment]::SetEnvironmentVariable(",
        "            $k, $null, [System.EnvironmentVariableTarget]::Process)",
        "    }",
