...
from core.update_worker import run_status_check, run_update
from widgets.component_row import ComponentRow
from widgets.header import build_header, _STRIP_BG, _MARGIN
...
class ComponentsView(ctk.CTkFrame):
    ...
    def _build_header(self) -> ctk.CTkFrame:
        ...
        # Attach the "last checked" label to the strip that build_header created.
        # The strip is always the second child gridded into the header frame.
    ...
    def _build_column_headers(self) -> ctk.CTkFrame:
        """Sticky header row whose columns align with ComponentRow's grid."""
        from widgets.component_row import COL_VER_W, COL_STATUS_W
        ...
        # Spacer matching checkbox cell (width 28 + padx 6+4 = 38)
    ...
    def _build_list(self) -> ctk.CTkScrollableFrame:
        ...
        # Group manifest items by section; custom components always go in "custom".
        ...
        for comp in self._settings.get("custom_components", []):
    ...
    def _add_section_divider(self, parent, title: str) -> None:
        """'MQ COMPONENTS ───────────────' header inside the scrollable list."""
    ...
    def _build_action_bar(self) -> ctk.CTkFrame:
        ...
        # Inner frame is centered horizontally via expand=True with no fill/side
    ...
    def _fit_window(self) -> None:
        """Resize the app window so the list gets at least 280px regardless of banner height."""
        LIST_MIN = 280
        ACTION_H = 56   # action bar + padding
        STRIP_H  = 60   # info strip below banner (two lines: title + author)
        COLHDR_H = 26   # column header row
    ...
    def _status_callback(self, cid: str, status: str, local: str | None, remote: str | None) -> None:
        """Marshal a worker status update onto the main thread."""
    ...
    def _on_checkbox_change(self, cid: str, checked: bool) -> None:
        selected: set[str] = set(self._settings.get("selected_components", []))
        if checked:
            selected.add(cid)
        else:
    ...
    def _on_custom_added(self, comp: dict) -> None:
        self._settings.setdefault("custom_components", []).append(comp)
        save_settings(self._settings)
...
class AddComponentDialog(ctk.CTkToplevel):
    """Stub dialog for adding a user-defined component."""
